...
def init_db():
    ...
    try:
        ...
        VALUES 
            ('parse', 0.0005, 0.0005, 100, '?????????????? ???????????? ??????????????????'),
            ('ai_parse', 0.005, 0.0075, 10, 'AI-???????????? ???????????? ??????????????????'),
            ('sender', 0.001, 0.001, 50, '???????????????? ???????????? ??????????????????'),
            ('invite', 0.002, 0.002, 20, '?????????????????????? ???????????? ????????????????????????')
...
def ai_score(req: AIScoreReq) -> Dict[str, Any]:
    try:
        ...
        system_prompt = (
            ...
            "?? ?????? ?????????? (???????? ????????) ?? ???????????? ?????????????????? ??????????.\n"
            "?????????? ???????????? ???????????????? JSON (?????? markdown, ?????? ??????????????????) ???????????? ?? ??????????????:\n"
            "{ \"results\": [ {\"id\":\"...\",\"score\":0-100,\"pass\":true/false,\"reason\":\"?????????????? 5-12 ????????\",\"flags\":[\"bot_like|spam_like|toxic|low_quality\"...]}, ... ] }\n"
            "?????????????? pass: true ???????? score >= min_score ?? ?????? flags bot_like/spam_like/toxic.\n"
            "Reason ???? ??????????????."
        )
